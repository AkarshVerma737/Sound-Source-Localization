...
for loop = 1:50
    ...
    for theta_src = 0:2:90  % Vary elevation from 0° to 90° in 2° steps
        ...
        for phi_src = 0:2:90
            ...
            beta = 0.2 + rand() * (0.8 - 0.2);      % reverberation time (RT_60)
            L = [5 4 6];
            ...
            %Stacking above two features along 1 dimension
            inputfeats_combined = cat(3, inputfeats_mag, inputfeats_ph);
    
            % Save feature files
            filename = sprintf('input_feats_azi%dele%d_%d.mat', phi_src, theta_src, loop);
            filepath = sprintf('E:/Beamforging/SH/Train Dataset/input_feats_azi%dele%d_%d.mat', phi_src, theta_src, loop);
            save(filepath, 'inputfeats_combined'); 

            % Store labels (filename, theta, phi, x, y, z)
            labels(end+1,:) = {filename, theta_src, phi_src, pos_src(1), pos_src(2), pos_src(3)};
        end
    end
end

% Save labels to CSV at the end
labels_table = cell2table(labels, 'VariableNames', {'filename','theta','phi','x','y','z'});
writetable(labels_table, 'E:/Beamforging/SH/labels.csv');
