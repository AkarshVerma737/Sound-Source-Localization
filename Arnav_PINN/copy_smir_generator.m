...
% Ensure s, L, and sphLocation are row vectors
disp(['Size of s: ', mat2str(size(s))]);
disp(['Size of L: ', mat2str(size(L))]);

% Check source position inside room bounds
if (~all(s <= L) || ~all(s >= 0))
    error('The source must be inside the room.');
end

% Check that the entire spherical array fits inside the room
if ~(all(sphLocation + sphRadius <= L) && all(sphLocation - sphRadius >= 0))
