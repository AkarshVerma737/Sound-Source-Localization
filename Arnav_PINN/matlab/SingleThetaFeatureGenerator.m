...
for smir_instance = 13:30
for run_instance = 1:3
test = '--------------'
smir_instance
run_instance

load(sprintf('/media/adityar/FFDOA-DANN/data/recording_train_theta_rand_%d_%d.mat',smir_instance,run_instance))
[M,rown,nframes,niterWithSpeech,nphi_src,ntheta_src] = size(features);
...
if(shmmagphaseshres == 1)
    ...
    saveshmmagphaseshres = '/media/adityar/FFDOA-DANN/data/shmmagphaseshres_train_theta_rand_%d_%d.mat';
end
...
for i=1:ntheta_src
    ...
    for j=1:nphi_src
		    ...
		    phi_labels(j,f_index) = 1;
		    theta_labels(i,f_index) = 1; 	
            	    f_index = f_index + 1;
		end
            end
    end
...
if(shmmagphaseshres == 1)
     ...
     save(sprintf(saveshmmagphaseshres,smir_instance,run_instance),'features','phi_labels','theta_labels','-v7.3');
end
