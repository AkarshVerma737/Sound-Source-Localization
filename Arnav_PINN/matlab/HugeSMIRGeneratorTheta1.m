...
beta_min = 0.250;
beta_max = 1;
h = zeros(length(theta_src),length(phi_src),M,nsample);

for i=1:length(theta_src)
    for j=1:length(phi_src)
            i
            length(theta_src)
            j
            length(phi_src)
	    ...
	    theta_iter = theta_src(i) + 1 - 2 * rand;
	    theta_iter = theta_iter * pi/180;	

	    phi_iter = phi_src(j) + 5 - 10 * rand;
...
save(sprintf('/media/adityar/FFDOA-DANN/data/SMIR_train_theta_rand_%d.mat',numfile),'h');
end
