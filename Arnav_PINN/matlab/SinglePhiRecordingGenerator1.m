...
for smir_instance = 1:30
for run_instance = 1:3
rng(10 * smir_instance + run_instance);

% load SMIR data into h
load(sprintf('/media/adityar/FFDOA-DANN/data/SMIR_train_phi_rand_%d.mat',smir_instance))
[nphi_src,ntheta_src,M,nsample] = size(h);
niterWithSpeech = 2;
nframes = 2;
...
features = zeros(M,rown,nframes,niterWithSpeech,ntheta_src,nphi_src);
...
save(sprintf('/media/adityar/FFDOA-DANN/data/recording_train_phi_rand_%d_%d.mat',smir_instance,run_instance),'features','-v7.3');
end
end
